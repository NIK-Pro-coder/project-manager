...
def generateReadme(path: str) :
	...
	if "#### Description:" in old_readme :
		print("Extracting description")
		desc = old_readme[old_readme.find("#### Description:") + 18:old_readme.find("---")].strip()
	else :
		desc = ""

	print("Adding name & short description")
	readme = f"## {meta["name"]}\n"
	readme += meta["desc"] + "\n"

	if desc != "" :
		print("Adding long description")
		readme += "\n#### Description:\n"
		readme += desc
		readme += "\n"
...
def gitCmd(path: str) :
	...
	if cmd == "link" :
		...
		if askyesno(f"/project{path[len(str(Path.home())):]}/git/commit | Would you like to autogenerate a README.md? ") :
			...
			print(yellow("If you'd like to have a longer description in your README.md"))
			print(yellow("add it under the '#### Description:' header"))
			input(yellow("If you'd like to edit the generated README.md do it now (ENTER to continue)"))
...
def game2048() :
	...
	grid = [
		[0 for x in range(4)] for i in range(4)
	]
	...
	def drawGrid() :
		for i in grid :
			print(" ------ ------ ------ ------")
			print("|", *[(f"{x:^4} |" if x != win_number else green(f"{x:^4}") + " |") if x != 0 else "     |" for x in i])
		print(" ------ ------ ------ ------")

	def spawnNum(pos: list[tuple[int, int]]) :
		num = 2 if random.random() <= .9 else 4
		p = random.choice(pos)

		grid[p[1]][p[0]] = num

	def compress(dir: str) :
		if dir == "a" :
			for n,i in enumerate(grid) :
				new = []
				for k in i :
		...
		if dir == "w" :
			for i in range(4) :
				for n in range(4) :
					for j in range(1,4) :
						if grid[j][n] != 0 and grid[j-1][n] == 0 :
							grid[j-1][n] = grid[j][n]
							grid[j][n] = 0
	...
	while True :
		...
		if any(any(x == win_number for x in i) for i in grid) :
			print(bold(green("You win")))
		...
		dir = autocomplete("Input a direction: ", ["a", "w", "s", "d"])
		compress(dir)
		merge(dir)
		compress(dir)
		merge(dir)
